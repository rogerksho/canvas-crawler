...
def getCourseName(course_id, courses):
...
def main():
    ...
    for i in courses:
        ...
        for j in ass_r.json():
            ...
            if due_date > datetime.now():
                todo.append(Task(j['id'], getCourseName(j['course_id'], courses), j['name'], due_date, j['has_submitted_submissions'], j['submission']['submitted_at'] is not None))
    ...
    for i, u in enumerate(todo_sorted):
        print("==============================")
        print(f"TASK {i}")
        if u.submitted:
